...
import "./navbar.css";
import Logo from "./assets/learnDuke.png";
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      axios
        .get(
          `${import.meta.env.VITE_SERVER_DEPLOY_URL}/getUser/${userdata.email}`
        )
        .then((response) => {
          if (response.data === "") {
  ...
  useEffect(() => {
    const saveUserDetails = async () => {
      ...
      if (email && name && accessToken) {
        ...
        await axios
          .get(`${import.meta.env.VITE_SERVER_DEPLOY_URL}/getUser/${email}`)
          .then((response) => {
            setPhoto(response.data.profilephoto.url);
        ...
        if (storedUser) {
          ...
          await axios
            .get(
              `${import.meta.env.VITE_SERVER_DEPLOY_URL}/getUser/${
    ...
    <div className="p-4 flex justify-between items-center sticky top-0 bg-white h-20 border-b border-gray-200  z-50">
      <div className="flex items-center space-x-4 lg:space-x-6">
        <img
          src={Logo}
          alt="Learn Duke Logo"
          className="h-16 mt-2 md:mt-1 mr-1 cursor-pointer"
          onClick={() => navigate("/")}
        />
      ...
      <div className="flex items-center space-x-6">
        {showMenu && (
          <div className="absolute top-0 w-full left-0  rounded-lg h-dvh backdrop-blur-sm z-10 flex flex-col lg:hidden animate-slide-left">
            <div
              ref={clickoutside}
              className="relative bg-white w-4/6 sm:w-3/6 md:max-w-80 h-full overflow-y-scroll p-5"
              style={{
            ...
            >
              {user ? (
                <div className="h-full flex flex-col justify-between ">
                  ...
                  <button
                    className="py-2 rounded-lg font-semibold bg-orange-500 text-white"
